...
const createCard = async (req, res) => {
    try {
        ...
        const cardCount = await cardModel.find().count()
        bodyData.cardNumber = "C" + (cardCount + 1)


        if (!cardType) {
            return res.status(400).send({
                status: false,
                message: "Please mention the Card type."
            })
        }
        if (cardType != "REGULAR" && cardType != "SPECIAL") {
            return res.status(400).send(
                {
                    status: false,
                    message: "cardType only followed enum ['REGULAR' , 'SPECIAL']"
                }
            )
        }
        ...
        if (custmCheck == "INACTIVE") {
            return res.status(400).send({
                status: false,
                message: "customer with this Id is Deleted."
            })
        }
        ...
        if (cardCheck) {
            return res.status(400).send({
                status: false,
                message: "A card is Already exist with this customerID."
            })
        }
